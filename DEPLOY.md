...
- **Кнопка СБП не появляется** — в коде она уже есть (при оплате заказа и при пополнении). Если в боте по-прежнему нет: 1) Убедитесь, что вы в папке проекта и закоммитили изменения: `git status`, при необходимости `git add .` и `git commit -m "СБП в меню"`. 2) Сделайте `git push origin main`. 3) В Railway откройте вкладку **Deployments** и дождитесь нового деплоя (после push он запускается сам). Если деплой не стартовал — нажмите **Redeploy** у последнего коммита.
- **TimeoutError при старте (init_db)** — подключение к PostgreSQL не успевает. Добавлены таймаут 60 с и до 8 попыток с паузой 8 с. В Variables можно задать `DB_CONNECT_TIMEOUT=90`, `DB_CONNECT_ATTEMPTS=10`, `DB_CONNECT_DELAY=10`.
...
- **429 Too Many Requests (CoinGecko)** — курс TON обновляется реже (интервал 10 мин по умолчанию). При 429 используется последний известный курс. Можно поставить `PRICE_UPDATE_INTERVAL=900` или `1200`, чтобы реже дергать API.
